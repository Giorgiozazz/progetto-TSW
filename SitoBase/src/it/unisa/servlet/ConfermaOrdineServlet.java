...
public class ConfermaOrdineServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (numRate == 0) {
            ...
            if (anticipo > totale) {
                request.setAttribute("error", "L'anticipo non può superare il totale dell'ordine (" + totale + " €).");
                request.getRequestDispatcher("/jsp/common/error.jsp").forward(request, response);
                return;
            }
        }
        ...
        if (utente == null) {
            session.setAttribute("loginMessage", "⚠️ Devi effettuare il login per finalizzare l'ordine.");
        ...
        try {
            ...
            // ** Qui aggiungo la scrittura nello snapshot delle auto vendute **
            for (ProductBean prodotto : carrello) {
                ...
                // salvo snapshot dell’auto venduta nella tabella separata
