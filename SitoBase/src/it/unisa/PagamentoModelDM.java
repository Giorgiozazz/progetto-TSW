...
public class PagamentoModelDM {
    ...
    String insertSQL = "INSERT INTO " + TABLE_NAME +
            " (ID_ORDINE, METODO_PAGAMENTO, ANTICIPO, NUM_RATE, IMPORTO_RATA, DATA_PAGAMENTO) " +
            "VALUES (?, ?, ?, ?, ?, ?)";

    try {
        connection = DriverManagerConnectionPool.getConnection();

        //  Assicura che sia in modalità manual commit se vuoi usare commit esplicitamente
        ...
        System.err.println("[ERRORE] Errore durante il salvataggio del pagamento: " + e.getMessage());
        if (connection != null) {
            try {
                connection.rollback();
                System.err.println("[DEBUG] Rollback effettuato.");
            } catch (SQLException rollbackEx) {
                System.err.println("[ERRORE] Rollback fallito: " + rollbackEx.getMessage());
            }
        }
        throw e; // rilancia per visibilità a monte
    } finally {
        if (preparedStatement != null) preparedStatement.close();
        if (connection != null) {
            connection.setAutoCommit(true); // ripristina stato predefinito
