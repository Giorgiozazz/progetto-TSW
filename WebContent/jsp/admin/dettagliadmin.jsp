...
<%
    // Recupera l'utente dalla sessione (modifica in base a come salvi il tuo utente)
    ...
    // Supponiamo che l'oggetto utente abbia un metodo isAdmin() o un attributo "ruolo"
    // Modifica in base alla tua implementazione reale
    boolean isAdmin = false;
    if (utente instanceof it.unisa.model.UtenteBean) { // esempio classe UserBean
        it.unisa.model.UtenteBean user = (it.unisa.model.UtenteBean) utente;
        isAdmin = user.isAdmin(); // o user.getRuolo().equals("admin")
    }

    if (!isAdmin) {
        // Non admin → redirect a pagina accesso negato o home
        response.sendRedirect(request.getContextPath() + "/jsp/common/accessoNegato.jsp");
        return;
    }
%>
