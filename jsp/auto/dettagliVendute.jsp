...
<body>
	...
	<%
	    ...
	    String idOrdine = (String) request.getAttribute("id");
	    if (idOrdine == null) {
	        idOrdine = ""; // fallback
	    }
	%>
	<div class="torna-wrapper">
	    ...
	    %>
	        <a href="<%= request.getContextPath() %>/jsp/admin/DettagliOrdineAdmin.jsp?id=<%= idOrdine %>" class="torna-catalogo">TORNA ALL'AREA ADMIN</a>
	    <%
	        } else {
	    %>
	        <a href="<%= request.getContextPath() %>/jsp/utente/DettagliOrdine.jsp?id=<%= idOrdine %>" class="torna-catalogo">TORNA A DETTAGLI ORDINE</a>
	    <%
	        }
	    %>
	</div>
