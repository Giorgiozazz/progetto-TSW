...
public class ProductModelDM implements ProductModel {
    ...
    public void doUpdate(ProductBean product, Connection connection) throws SQLException {
        ...
        String updateSQL = "UPDATE " + TABLE_NAME + " SET "
                ...
                + "PORTE=?, TIPO=?, CLIMATIZZATORE=?, RUOTEMOTRICI=?, SCHERMO_INTEGRATO=?, CILINDRATA=?, POSTI=?, "
                + "TIPO_CAMBIO=?, RAPPORTI=?, POTENZAELETTRICA=?, TRAZIONE=?, PESO=?, COLORE=?, POTENZA=?, "
                + "IMAGEPATH=?, CF=?, VENDUTA=?, ID_ORDINE=? "
                + "WHERE VIN=?";
    ...
    public synchronized List<ProductBean> getTop6Cars() throws SQLException {
        ...
        // Aggiungo IMAGEPATH alla query
        String sql = "SELECT VIN, MARCA, MODELLO, PREZZO, IMAGEPATH FROM " + TABLE_NAME + " WHERE VENDUTA = FALSE LIMIT 6";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                car.setImmagine(rs.getString("IMAGEPATH"));  
                cars.add(car);
            }
            rs.close();
        } finally {
